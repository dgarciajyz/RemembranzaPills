...
module.exports.putAppointment = function (req, res, next) {
    //Parameters
    console.log(req)
    connection.query('UPDATE Appointment SET ? WHERE place = ' + req.undefined.value.place + " AND hour = " + req.undefined.value.place, [req.undefined.value], function (error, results, fields) {
        if (error) throw error;
        res.send({
            message: results
        });
